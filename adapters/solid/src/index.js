...
export function createAdapter() {
  return {
    ...
    scaffold(name, options) {
      return scaffoldSolid(name, options)
    }
  }
}
...
async function scaffoldSolid(name, options) {
  ...
  writeFileSync(join(projectDir, 'package.json'), JSON.stringify({
    ...
    devDependencies: { vite: '^6.1.0', 'vite-plugin-solid': '^2.11.0', typescript: '^5.7.0', vitest: '^3.0.0', eslint: '^9.20.0', '@eslint/js': '^9.20.0', prettier: '^3.5.0' }
  }, null, 2))

  writeFileSync(join(projectDir, 'tsconfig.json'), JSON.stringify({
    compilerOptions: {
      target: 'ESNext', module: 'ESNext', moduleResolution: 'bundler', strict: true, jsx: 'react-jsx',
      jsxImportSource: 'solid-js', skipLibCheck: true, noEmit: true, isolatedModules: true
    },
    include: ['src']
  }, null, 2))
...
`)
  ...
  writeFileSync(join(projectDir, 'src', 'index.tsx'), `import { render } from 'solid-js/web'
import App from './App'

render(() => <App />, document.getElementById('root')!)
`)

  writeFileSync(join(projectDir, 'src', 'App.tsx'), `import { createSignal } from 'solid-js'
