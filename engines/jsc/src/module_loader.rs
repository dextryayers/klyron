...
impl JSCModuleLoader {
    ...
    pub fn resolve(&self, specifier: &str, base: &str) -> Result<String, JSCError> {
        ...
        if resolved.exists() {
            ...
            Err(JSCError::CompileError(format!("Module not found: {}", specifier)))
        }
    }

    pub fn load(&self, path: &str) -> Result<String, JSCError> {
        ...
        std::fs::read_to_string(path)
            .map_err(|e| JSCError::CompileError(format!("Failed to load {}: {}", path, e)))
    }
