...
impl BoaModuleLoader {
    ...
    pub fn resolve(&self, specifier: &str, base: &str) -> Result<String, BoaError> {
        ...
        if resolved.exists() { Ok(resolved.to_string_lossy().to_string()) }
        else { Err(BoaError::CompileError(format!("Module not found: {specifier}"))) }
    }

    pub fn load(&self, path: &str) -> Result<String, BoaError> {
        let modules = self.modules.lock().unwrap();
        if let Some(source) = modules.get(path) { return Ok(source.clone()); }
        drop(modules);
    ...
    pub fn register(&self, name: &str, source: &str) {
        self.modules.lock().unwrap().insert(name.to_string(), source.to_string());
    }
}
