...
impl FrameworkAdapter for ReactAdapter {
    ...
    fn detect(&self, dir: &Path) -> bool {
        ...
        let has_react = dir.join("package.json").exists().then(|| {
            ...
            content.map(|c| c.contains("\"react\"") || c.contains("\"react-dom\"")).unwrap_or(false)
        }).unwrap_or(false);
        has_vite && has_react
    }
  ...
  "include": ["src"]
        ...
        std::fs::write(
            project_dir.join("tsconfig.node.json"),
            r#"{
  "compilerOptions": {
    "target": "ES2022",
    "lib": ["ES2023"],
...
export default defineConfig({
  ...
  server: {
    ...
    host: true,
        ...
        std::fs::write(
            project_dir.join("vitest.config.ts"),
...
export default App"#,
        ...
        std::fs::write(
            project_dir.join("src/pages/Home.tsx"),
            klyron_template::TemplateEngine::render(
                r#"function Home() {
  return (
    <div>
...
"#,
        ...
        std::fs::write(
            project_dir.join("src/test/setup.ts"),
...
"#,
  ...
  "printWidth": 100
        ...
        std::fs::write(
            project_dir.join("eslint.config.js"),
