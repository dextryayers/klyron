...
impl FrameworkAdapter for VueAdapter {
    ...
    fn supported_versions(&self) -> Vec<&'static str> { vec!["3.4", "3.5"] }
    fn default_version(&self) -> &'static str { "3.5" }
  ...
  "include": ["src/**/*.ts", "src/**/*.tsx", "src/**/*.vue"]
        ...
        std::fs::write(project_dir.join("tsconfig.node.json"),
            r#"{
  "compilerOptions": {
    "target": "ES2022",
    "lib": ["ES2023"],
...
app.mount('#app')"#)?;
        ...
        std::fs::write(project_dir.join("src/App.vue"),
            r#"<template>
  <router-view />
</template>
