...
impl FrameworkAdapter for NextAdapter {
    ...
    fn supported_versions(&self) -> Vec<&'static str> { vec!["14.0", "15.0"] }
    fn default_version(&self) -> &'static str { "15.0" }
...
"#, vars))?;
        ...
        std::fs::write(project_dir.join("app/globals.css"),
            r#"@tailwind base;
@tailwind components;
@tailwind utilities;
...
"#)?;
        ...
        std::fs::write(project_dir.join("app/not-found.tsx"),
            r#"export default function NotFound() {
  return <div><h1>404 - Page Not Found</h1></div>
}
"#)?;

        std::fs::write(project_dir.join("app/error.tsx"),
            r##""use client"
...
"##)?;
        ...
        std::fs::write(project_dir.join("app/loading.tsx"),
            r#"export default function Loading() {
  return <div>Loading...</div>
}
"#)?;

        std::fs::write(project_dir.join("next-env.d.ts"),
