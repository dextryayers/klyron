...
use crate::cache::resolve_cache_get;
use crate::cache::resolve_cache_insert;
...
impl ModuleResolver {
    ...
    pub fn resolve_specifier(specifier: &str, base: &Url) -> Result<Url, LoaderError> {
        ...
        if specifier.starts_with("file:") {
            ...
            return PathBuf::from(path)
                .canonicalize()
                .map_err(|e| LoaderError::ModuleNotFound(format!("{path}: {e}")))
                .and_then(|p| {
                    Url::from_file_path(&p)
                        .map_err(|_| LoaderError::InvalidSpecifier(p.to_string_lossy().into()))
                });
        }
    ...
    fn resolve_node_module_entry(dir: &Path) -> Option<PathBuf> {
        ...
        for ext in &[
            ".js", ".mjs", ".cjs", ".json", ".ts", ".jsx", ".tsx", ".wasm", ".node",
        ] {
    ...
    pub fn resolve(&self, specifier: &str, base: &Path) -> Result<ModuleResolution, LoaderError> {
        ...
        if let Some(cached) = resolve_cache_get(&cache_key) {
            let format = Self::detect_format(&cached);
            let kind = Self::detect_kind(&cached);
            let url = Url::from_file_path(&cached).unwrap_or_else(|_| Url::parse("file:///").unwrap());
        ...
        if let Ok(ref resolution) = result {
            resolve_cache_insert(&cache_key, resolution.resolved_path.clone());
        }
    ...
    fn resolve_inner(
        ...
        specifier: &str,
        base: &Path,
    ) -> Result<ModuleResolution, LoaderError> {
        ...
        if let Some(mapped) = self.resolve_import_map(specifier, referrer) {
            let url = Url::parse(&mapped).map_err(|e| {
                LoaderError::InvalidSpecifier(format!("Import map mapped to invalid URL: {e}"))
            })?;
        ...
        if specifier.starts_with("file://") {
            ...
            let path = url.to_file_path().map_err(|_| {
                LoaderError::InvalidSpecifier(format!("Invalid file URL: {specifier}"))
            })?;
            let format = Self::detect_format(&path);
            let kind = Self::detect_kind(&path);
            return Ok(ModuleResolution {
        ...
        let pkg_json = 'find_pkg: {
            if resolved.starts_with("node_modules")
                || resolved.to_string_lossy().contains("node_modules")
            {
...
pub fn cjs_to_esm(source: &str) -> String {
    ...
    while i < lines.len() {
        ...
        if trimmed.starts_with("module.exports = ") || trimmed == "module.exports =" {
            ...
            let rest = trimmed.strip_prefix("module.exports = ").unwrap_or("");
            if rest.is_empty() {
            ...
            if let Some((name, val)) = rest.split_once(" = ") {
                let prefix = &line[..line.len() - trimmed.len()];
                out.push_str(&format!("{prefix}export const {name} = {val}"));
                out.push('\n');
            } else {
                out.push_str(line);
                out.push('\n');
...
pub fn esm_to_cjs(source: &str) -> String {
    ...
    while i < lines.len() {
        ...
        if let Some(rest) = trimmed.strip_prefix("export default ") {
            ...
            if let Some((name_val, val)) = rest.split_once(" = ") {
                let prefix = &line[..line.len() - trimmed.len()];
                out.push_str(&format!("{prefix}exports.{name_val} = {val}"));
                out.push('\n');
            } else {
                out.push_str(line);
                out.push('\n');
            ...
            if let Some(spec) = trimmed.strip_prefix("import ") {
                if let Some(module) =
                    spec.strip_prefix('"').or_else(|| spec.strip_prefix('\''))
                {
