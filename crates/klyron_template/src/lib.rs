      ...
      if entry.path().is_file() {
        if let Some(ext) = entry.path().extension().and_then(|e| e.to_str()) {
          if ext == "html" || ext == "njk" || ext == "hbs" {
            let name = entry.path().strip_prefix(dir)
    ...
    if let Some(caps) = extends_re.captures(&result) {
      ...
      if let Some(layout) = self.partials.get(layout_name) {
        ...
        for block_cap in block_re.captures_iter(template) {
          ...
          let block_placeholder = format!("{{% block {block_name} %}}...{{% endblock %}}");
          result = result.replace(&block_placeholder, block_content);
        }
      }
    }
    ...
    loop {
      let new_result = for_re.replace_all(&result, |caps: &regex::Captures| {
        ...
        for item in &items {
          ...
          let rendered = self.render(body, &item_vars);
...
mod tests {
  ...
  fn test_render_multiple_vars() {
    ...
    let result = engine.render(r#"{"name": "{{ name }}", "version": "{{ version }}"}"#, &vars);
  ...
  fn test_load_partials_dir() {
    ...
    assert!(engine.partials.contains_key("header"));
    assert!(engine.partials.contains_key("footer"));
    let _ = std::fs::remove_dir_all(&dir);
  }
}
