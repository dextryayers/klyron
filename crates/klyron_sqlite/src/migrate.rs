...
impl Migrator {
    ...
    pub fn run(&self, db: &SqliteDb) -> Result<Vec<String>> {
        self.ensure_migrations_table(db)?;

        let applied = self.get_applied_versions(db)?;
        let mut executed = Vec::new();

        for (version, migration) in &self.migrations {
            if !applied.contains(version) {
                let start = Instant::now();
                db.execute(&migration.sql_up, &[])?;
                self.record_migration(db, version, &migration.description, start.elapsed())?;
    ...
    pub fn rollback(&self, db: &SqliteDb, target_version: &str) -> Result<Vec<String>> {
        let applied = self.get_applied_versions(db)?;
        ...
        for (version, migration) in self.migrations.iter().rev() {
            ...
            if let Some(ref down) = migration.sql_down {
                db.execute(down, &[])?;
                self.remove_migration(db, version)?;
    ...
    fn ensure_migrations_table(&self, db: &SqliteDb) -> Result<()> {
        let sql = format!(
    ...
    fn get_applied_versions(&self, db: &SqliteDb) -> Result<Vec<String>> {
    ...
    fn record_migration(&self, db: &SqliteDb, version: &str, description: &str, duration: std::time::Duration) -> Result<()> {
        let sql = format!(
            "INSERT INTO {} (version, description, duration_ms) VALUES (?1, ?2, ?3)",
            self.table_name
        );
        db.execute(&sql, &[
            rusqlite::types::Value::Text(version.to_string()),
            rusqlite::types::Value::Text(description.to_string()),
            rusqlite::types::Value::Integer(duration.as_millis() as i64),
        ])?;
        Ok(())
    }

    fn remove_migration(&self, db: &SqliteDb, version: &str) -> Result<()> {
        let sql = format!("DELETE FROM {} WHERE version = ?1", self.table_name);
        db.execute(&sql, &[rusqlite::types::Value::Text(version.to_string())])?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_add_migration() {
        ...
        m.add_migration("001", "create_users", "CREATE TABLE users (id INTEGER PRIMARY KEY)", None);
    ...
    #[test]
    fn test_migrate_run_empty() {
        let db = crate::SqliteDb::open_in_memory().unwrap();
        let m = Migrator::new();
        let executed = m.run(&db).unwrap();
        assert!(executed.is_empty());
    }

    #[test]
    fn test_migrate_run_with_migrations() {
        let db = crate::SqliteDb::open_in_memory().unwrap();
        let mut m = Migrator::new();
        m.add_migration("001", "create_table",
            "CREATE TABLE test (id INTEGER PRIMARY KEY, name TEXT)", None);
        let executed = m.run(&db).unwrap();
        assert_eq!(executed, vec!["001"]);
