...
use fs::{event_path, is_ignored_by_patterns};
...
impl FileWatcher {
        ...
        F: Fn(WatchEvent) + Send + 'static,
    {
        ...
        thread::spawn(move || {
            while running_clone.load(Ordering::SeqCst) {
                match rx.try_recv() {
                    Ok(event) => {
                        if !is_ignored_by_patterns(event_path(&event), &ignore) {
                            callback(event);
                        }
                    }
        ...
        F: Fn(WatchEvent) + Send + 'static,
    {
        ...
        thread::spawn(move || {
            ...
            loop {
                ...
                let files = crate::fs::scan_files(&paths, recursive, &ignore);
        ...
        F: Fn(HmrUpdate) + Send + 'static,
    {
        ...
        thread::spawn(move || {
            ...
            debouncer.run(rx, running_clone, ignore, callback);
        });
