...
impl RubyWasmEngine {
  ...
  pub fn init(&self) -> Result<(), String> {
    ...
    let wasm_path = self.config.ruby_path.as_deref().unwrap_or("/usr/lib/klyron/ruby.wasm");
    let engine = wasmtime::Engine::default();
    let module = wasmtime::Module::from_file(&engine, wasm_path)
      .map_err(|e| format!("Failed to load Ruby-WASM module: {e}"))?;
    ...
    let instance = linker.instantiate(&mut store, &module)
      .map_err(|e| format!("Ruby-WASM instance: {e}"))?;
...
impl RubyEngine for RubyWasmEngine {
  ...
  fn call_method(&self, receiver: Option<&str>, method: &str, args: &[serde_json::Value]) -> Result<serde_json::Value, String> {
    ...
    if !result.status.success() && stdout.trim().is_empty() {
      return Err(format!("Ruby method call failed (exit {}): {}", result.status.code().unwrap_or(-1), String::from_utf8_lossy(&result.stderr)));
    }
